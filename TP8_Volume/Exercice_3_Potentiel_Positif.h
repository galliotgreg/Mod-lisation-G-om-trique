...
void render_scene()
{
	...
	point3 centreSphere1 = point3(-32.0, 10.0, 0.0);
	int rayon1 = 100;
	point3 centreSphere2 = point3(30.0, 0.0, 0.0);
	int rayon2 = 100;
	point3 centreSphere3 = point3(0.0, -20, 0.0);
	int rayon3 = 100;
	point3 centreSphere4 = point3(0.0, 25.0, 0.0);
	int rayon4 = 100;
	...
	for (int i = -25; i < 25; i++) {
		for (int j = -25; j < 25; j++) {
			for (int k = -25; k < 25; k++) {
				...
				if (potentiel(point3(i, j, k), TabSpheres) > 0.09) {
					glColor3f(0.25, 0.25, 0.0);
					glBegin(GL_LINE_STRIP);
					for (point3 point : TabLeft) {
						//ABCD
...
int main(int argc, char **argv)
{
	...
	glutCreateWindow("Volumes paramètriques");
	init();
